"""Error metrics for comparing predicted vs. actual arrays.

All functions accept numpy arrays of identical shape and return a scalar float.
"""
...
def l_inf(actual: np.ndarray, predicted: np.ndarray) -> float:
    ...
    _validate_shape(actual, predicted)
    return float(np.max(np.abs(np.asarray(predicted) - np.asarray(actual))))


def relative_l2(actual: np.ndarray, predicted: np.ndarray, eps: float = 1e-16) -> float:
    """Relative L2 error: RMSE / RMS(actual)."""
    _validate_shape(actual, predicted)
    actual, predicted = np.asarray(actual, dtype=float), np.asarray(predicted, dtype=float)
...
def relative_l_inf(actual: np.ndarray, predicted: np.ndarray, eps: float = 1e-16) -> float:
    ...
    _validate_shape(actual, predicted)
    actual, predicted = np.asarray(actual, dtype=float), np.asarray(predicted, dtype=float)
    return float(np.max(np.abs(predicted - actual)) / (np.max(np.abs(actual)) + eps))


def gradient_l2_error(actual: np.ndarray, predicted: np.ndarray, eps: float = 1e-16) -> float:
    """Relative L2 error of gradients (computed via np.gradient)."""
    _validate_shape(actual, predicted)
    actual, predicted = np.asarray(actual, dtype=float), np.asarray(predicted, dtype=float)
...
def relative_energy_error(actual: np.ndarray, predicted: np.ndarray, eps: float = 1e-16) -> float:
    """Relative energy error: |E_pred - E_actual| / E_actual."""
    _validate_shape(actual, predicted)
    actual, predicted = np.asarray(actual, dtype=float), np.asarray(predicted, dtype=float)
    energy_a = np.sum(actual ** 2)
    energy_p = np.sum(predicted ** 2)
    return float(np.abs(energy_p - energy_a) / (energy_a + eps))
...
def pearson_correlation(actual: np.ndarray, predicted: np.ndarray) -> float:
    ...
    if std_a == 0 or std_p == 0:
        return np.nan if std_a != std_p else 1.0
    return float(np.mean(a_c * p_c) / (std_a * std_p))


def robust_median_absolute_error(actual: np.ndarray, predicted: np.ndarray) -> float:
    """Median Absolute Error (robust to outliers)."""
    _validate_shape(actual, predicted)
    return float(np.median(np.abs(np.asarray(predicted) - np.asarray(actual))))
